...
(function () {
  ...
  function countdown() {
    ...
    const e = CFG.event || {};
    const target = new Date(e.startISO || (e.dateISO + "T08:00:00-03:00")).getTime();
    ...
    grid.innerHTML = units.map((u) =>
      `<div class="countdown__unit"><span class="countdown__num" data-k="${u.k}">00</span><span class="countdown__label">${u.label}</span></div>`).join("");
    const nums = {};
    units.forEach((u) => { nums[u.k] = grid.querySelector(`[data-k="${u.k}"]`); });

    const pad = (n) => String(n).padStart(2, "0");
    const set = (k, val) => {
      const el = nums[k]; if (!el) return;
      const str = k === "d" ? String(val) : pad(val);
      if (el.textContent !== str) {
        el.textContent = str;
        if (!prefersReduced) { el.classList.remove("is-tick"); void el.offsetWidth; el.classList.add("is-tick"); }
      }
    };
    ...
    const tick = () => {
      ...
      if (diff <= 0) {
        clearInterval(timer);
        root.innerHTML = `<p class="countdown__done">O dia chegou. Bem-vindo à casa.</p>`;
        return;
      }
